...
static bool test_new_classes_without_session(RzCore *core) {
	char *n = rz_core_cmd_str(core, "fridaNj");
	mu_assert_notnull(n, "fridaNj returns output");
	mu_assert_streq(n,
		"{\"ok\":false,\"error\":{\"code\":\"invalid_target\",\"message\":\"no session is open\"}}\n",
		"fridaNj without an open session reports the precondition failure");
	RZ_FREE(n);
	n = rz_core_cmd_str(core, "fridaNj start");
	mu_assert_notnull(n, "fridaNj start returns output");
	mu_assert_streq(n,
		"{\"ok\":false,\"error\":{\"code\":\"invalid_target\",\"message\":\"no session is open\"}}\n",
		"fridaNj start without an open session reports the precondition failure");
	RZ_FREE(n);
	mu_end;
}
...
static bool test_rn_without_session(RzCore *core) {
	...
	mu_assert_streq(n,
		"{\"ok\":false,\"error\":{\"code\":\"invalid_target\",\"message\":\"no session is open\"}}\n",
		"fridaRNj without an open session reports precondition failure");
	RZ_FREE(n);
	n = rz_core_cmd_str(core, "fridaRNj on");
	mu_assert_notnull(n, "fridaRNj on returns output");
	mu_assert_streq(n,
		"{\"ok\":false,\"error\":{\"code\":\"invalid_target\",\"message\":\"no session is open\"}}\n",
...
static bool test_flag_modules_without_session(RzCore *core) {
	char *n = rz_core_cmd_str(core, "fridafj");
	mu_assert_notnull(n, "fridafj returns output");
	mu_assert_streq(n,
		"{\"ok\":false,\"error\":{\"code\":\"invalid_target\",\"message\":\"no session is open\"}}\n",
		"fridafj without an open session reports precondition failure");
	RZ_FREE(n);
	mu_end;
}
